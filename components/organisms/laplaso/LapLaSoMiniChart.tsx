...
const ThienBan = memo(function ThienBan({ result }: { result: TuViEngineResult }) {
  ...
  return (
    <div className="glass-border-panel-soft rounded-[20px] p-4 h-full flex flex-col">
      ...
      <div className="grid grid-cols-2 gap-1.5 flex-1">
        {info.map(([label, value]) => (
          <div key={label} className="glass-border-panel-soft rounded-lg px-2 py-1.5">
...
export default function LapLaSoMiniChart({ result }: Props) {
  ...
  const palaceByBranch = useMemo(() =>
    Object.fromEntries(result.palaces.map((p) => [p.branch, p])),
  [result.palaces]);
  ...
  return (
    <>
      <div
        className="glass-border-panel col-span-12 lg:col-span-8 rounded-xl p-5 relative overflow-hidden"
      >
        <div className="absolute top-0 right-0 p-6 opacity-[0.15] pointer-events-none select-none">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="glass-border-panel-soft text-xs text-outline px-3 py-1 rounded-full">
          ...
          >
            {RING_LAYOUT.map((branch, index) => {
              ...
              if (!palace) return <div key={index} className="glass-border-panel-soft rounded-xl min-h-[130px]" />;
